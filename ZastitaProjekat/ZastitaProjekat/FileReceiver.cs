...
public class FileReceiver
{
    ...
    public void StartGui()
    {
        ...
        Log($"[Receiver] Cekaju se fajlovi na portu {port}...");
    }
    ...
    private void ListenLoopGui(CancellationToken token)
    {
        try
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                using (client)
                {
                    ...
                    {
                        Log("[Receiver] Greska: " + ex.Message);
                    }
                }
            }
        }
        ...
        {
            Log("[Receiver] Greska listenera: " + ex.Message);
        }
        finally
        {
    ...
    private void HandleIncomingFileGui(NetworkStream stream)
    {
        ...
        if (fileSize < 0)
            throw new InvalidDataException("Negativna velicina fajla nije dozvoljena.");
        ...
        while (remaining > 0)
        {
            ...
            if (read <= 0) throw new EndOfStreamException("Neocekivan kraj toka pri prijemu sadržaja.");
        ...
        if (!ok)
        {
            ...
            Log("[Receiver] Heš NE ODGOVARA. Fajl je ostecen ili izmenjen! Privremeni fajl obrisan.");
            return;
        }
